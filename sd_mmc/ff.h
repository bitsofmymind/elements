...
typedef struct {
	uint8_t pd;	/* Physical drive# */
	uint8_t pt;	/* Partition # (0-3) */
} PARTITION;
...
#if _MAX_SS != 512
	WORD	ssize;			/* uint8_ts per sector (512,1024,2048,4096) */
#endif
#if _FS_REENTRANT
...
#if _USE_FORWARD
FRESULT f_forward (FIL*, UINT(*)(const uint8_t*,UINT), UINT, UINT*);	/* Forward data to the stream */
#endif

#if _USE_MKFS
FRESULT f_mkfs (uint8_t, uint8_t, UINT);					/* Create a file system on the drive */
#endif
